...
echo "#SBATCH -o $gitdir/hpc/outputs/$fn.out" >> "$gitdir/hpc/slurm_scripts/$fn.out"

echo "julia -e 'import Pkg; Pkg.add(\"DrWatson\"); using DrWatson; @quickactivate; Pkg.instantiate()'" >> "$gitdir/hpc/slurm_scripts/$fn.sh"
echo "julia - t 50 scripts/calc_complexity_entropy_mackey_glass.jl" >> "$gitdir/hpc/slurm_scripts/$fn.sh"
